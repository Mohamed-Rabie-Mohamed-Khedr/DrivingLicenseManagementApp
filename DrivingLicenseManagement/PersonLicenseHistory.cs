...
namespace DrivingLicenseManagement
{
    ...
    {
        private int _LicenseID, _PersonID;
        private bool _tab1IsActive;
        ...
        public PersonLicenseHistory(int LicenseID, int PersonID, bool tab1IsActive = true)
        {
            ...
            _PersonID = PersonID;
            _tab1IsActive = tab1IsActive;
        }

        private void PersonLicenseHistory_Load(object sender, EventArgs e)
        {
            if (!_tab1IsActive) tabControl1.SelectedIndex = 1;
            DriverInfo driverLicenseInfo = new DriverInfo()
            {
                Dock = DockStyle.Top
            };
            driverLicenseInfo.LoadDriverInfo(_LicenseID);
            this.Controls.Add(driverLicenseInfo);

            DataTable d = MyDB.GetPersonLicenseHistory(_PersonID);
            if (d != null)
            {
            ...
            InternationalLicense il = MyDB.GetInternationalLicense(_LicenseID);
            if (il != null)
